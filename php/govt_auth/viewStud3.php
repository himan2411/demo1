...
<body>
  ...
  <form method="POST" action="">
	<div class="input-field col s3">
    <select multiple name="Region[]" onchange="this.form.submit()" > 
...
<div class="input-field col s3">
	...
	<?php
$sql2 = "SELECT * FROM `zone_mapping` WHERE `Id` is not null";
...
?>
      ...
      <option value="<?php echo $row2['District'];?>"><?php echo $row2['District'];?></option>
      <?php
				 }
	  ?>
 ...
 if(isset($_POST['submit']))
	{
		...
		*/
		...
		$sql1 = "SELECT DISTINCT `Index_No` FROM `extra_cur` Where `Index_No` IS NOT NULL ";
		
		$sql3 = "SELECT DISTINCT `Aadhar_No` FROM `extra_cur` Where `Gr_No` IS NOT NULL ";
		
		$sql = "SELECT * FROM `student` where `Aadhar_No` IS NOT NULL ";
